#ifndef __MYBOUTONACTIF_H__
#define __MYBOUTONACTIF_H__
...
class CMyBoutonActif : public CMyBoutonBase {
    private:
    ...
    public:
        CMyBoutonActif(CConfig& cfg, TFT_eSPI& tft, XPT2046_Touchscreen& _touch, unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int color, String name) : 
        CMyBoutonBase(tft, _touch, x, y, w, h, color, name)/*,
        mConfig(&cfg), mTft(tft), muiPosX(x), muiPosY(y), muiWidth(w), muiHight(h), muiColor(color), msNom(name)*/ {}
